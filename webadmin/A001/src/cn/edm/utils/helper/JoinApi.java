...
public class JoinApi {
    ...
    public static String snatch(String filePath) {
        ...
        try {
            ...
            HttpGet get = new HttpGet("http://" + Property.getStr(Config.DELIVERY_API) + filePath);
            HttpResponse response = client.execute(get);
            HttpEntity entity = response.getEntity();
            if (entity != null) {
                reader = new BufferedReader(new InputStreamReader(entity.getContent(), "UTF-8"));
                writer = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream(Webs.rootPath() + PathMap.JOIN_API.getPath() + name), "UTF-8")));
                String line = null;
	...
	public static String read(String filePath) {
		...
		PrintWriter writer = null;
		try {
			path = Property.getStr(Config.DELIVERY_API);
			String name = StringUtils.substring(filePath, StringUtils.lastIndexOf(filePath, "/") + 1);
