...
public class Files {
	...
	public static boolean createTask(Integer taskId) {
		 String dir = Webs.webRoot() + PathMap.SCAN_TASK.getPath();
	...
	public static boolean createPlan(Integer planId) {
		String dir = Webs.webRoot() + PathMap.SCAN_PLAN.getPath();
		if(!Files.exists(dir)) Files.make(dir);
	...
	/**
	 * 创建以筛选ID为名的文件，扫描时扫描该名的文件
	 * @param selectionId
	 * @return
	 */
	public static boolean createSelection(Integer selectionId) {
		String dir = Webs.webRoot() + PathMap.SCAN_SELECTION.getPath();
		if(!Files.exists(dir)) Files.make(dir);
		String fileName = selectionId + ".txt";
		String filePath = dir + fileName;
		return createFile(filePath);
	}
	
	/**
	 * 扫描以ID为名文件，如以任务ID，周期ID等ID为名的文件
	 * @param path
	 * @return
	 */
	public static List<Integer> scan(String path) {
		List<Integer> taskIds = Lists.newArrayList();
		File dir = new File(path);
        if (dir.exists()) {
            File[] files = dir.listFiles();
            int length = files.length;
            Integer scanSize = Property.getInt(Config.SCAN_SIZE);
            int total = scanSize==null ? 50 : scanSize; // 最多一次取  scanSize 个文件，默认为50个
            int len = length;
            // 如果文件不够100个则取文件的全部
            if(length > total ) {
            	len = total;
            ...
            // 周期任务不限制扫描文件个数
            if(path.contains("/scan/plan/")){
            	len = length;
            }
            // 限制查询个数，避免oracle的in条件查询出错
            if(len > 999){
            	len = 999;
            }
            for(int i=0; i<len; i++) {
                ...
                if(name != null ) taskIds.add(Integer.valueOf(name));
            }
        }
		return taskIds;
	}
