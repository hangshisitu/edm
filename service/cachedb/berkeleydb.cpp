...
void BerkeleyDB::bdb_env_init(const char* envpath)
{
	...
	ret = BerkeleyDB::dbenv->open(
	        BerkeleyDB::dbenv,
	        envpath,
	        DB_CREATE | DB_INIT_LOCK | DB_INIT_MPOOL, //DB_INIT_CDB OR DB_INIT_LOCK
	        0);
...
int BerkeleyDB::cursor_traversal_delete(traversal_func_pt func, void* param, int flag)//DB_NEXT,DB_PREV
{
    if (!func)
        return -1;
