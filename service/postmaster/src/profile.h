...
{
    static inline void set(__attribute__((unused)) std::stringstream &ss) {};
};
...
public:
    ...
    int get_pair(const char* node, key_map_t& target);
    int get_keys(const char* name, std::list<keys_t>& keyslist);
...
private:
    node_map_t nodes_;
    std::map<node_t, std::list<keys_t> > keyslist_;
};
