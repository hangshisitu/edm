...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<script type="text/javascript" src="${ctx}/resources/script/report/customerTaskStatList.js?${version}"></script>
  ...
  <body>
    <div id="main" class="clearfix" style="position:relative">
		...
		<form id="ListForm1" action="${ctx}/customerReport/taskList" method="get">
		...
		<div class="ribbon clearfix">
			...
			<ul class="btn">
				<li>
					<input type="button" class="groovybutton" value="返 回" title="返回" onclick="back('${ctx}/customerReport/list')">
				</li>
	...
	<h1 class="title"><!-- <span class="search_top f_r"><a class="search_hide" href="javascript:void(0)">查找</a></span> -->&nbsp;<span></span></h1>
		<form id="ListForm" action="${ctx}/customerReport/taskList" method="get">
			<%--<ul class="search clearfix m_b00 no_display">--%>
				<ul class="search clearfix m_b10">
				<li>任务名称</li>
				<li>
		...
		<div class="ribbon clearfix">
			...
			<ul class="btn">
				...
				<li>
					<input type="button" class="groovybutton" value="导 出" title="导出" onclick="exportTaskStatNew('${ctx}/taskReport/exportTaskStatNew','${corpId}')">
				</li>
				<li>
		...
		<table width="100%" cellspacing="0" cellpadding="0" border="0" class="main_table">
			<tbody>
				...
				<c:forEach items="${pagination.recordList}" var="r" varStatus="i">
					<c:choose>
						<c:when test="${r.planId != null}">
							<tr>
								<td><input type="checkbox" name="checkedIds" value="${r.taskId}" data-value="planTask"/></td>
								...
								<td class="edit">
									<ul class="f_ul">
										<li><a href="javascript:detail('${ctx}/plan/planDetailStat/planReportView?planId=${r.planId}')">报告</a></li>
									</ul>
								</td>
							</tr>
						</c:when>
						<c:otherwise>
							<tr>
								<td><input type="checkbox" name="checkedIds" value="${r.taskId}" data-value="singleTask" /></td>
								<td title="${r.taskName}">${r.taskName}</td>
								<td title="${r.corpPath}">${r.corpPath}</td>
								<td>${r.userId}</td>
								<td>单项任务</td>
		...
		<input type="hidden" name="corpId" id="corpId" value="${corpId}">
		<!-- 0，1 单项与测试任务 -->
		<input type="hidden" name="typeStr" id="typeStr" value="0,1,2">
		<input type="hidden" name="fromFlag" id="fromFlag" value="customer">
	</form>
		<input type="hidden" name="msg" id="msg" value="${message}">
		<input type="hidden" name="listUrl" id="listUrl" value="${ctx}/customerReport/taskList">
		<br />
	<br />
	<br />
	</div>
