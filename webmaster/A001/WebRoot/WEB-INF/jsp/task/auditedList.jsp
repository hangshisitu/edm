...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    <div id="main" class="clearfix" style="">
		...
		<form id="ListForm" action="${ctx}/task/auditedList" method="get">
		...
		<ul class="tab clearfix">
			<li><a href="${ctx}/task/unauditList">待审核任务</a></li>
			<li  class="now"><a href="${ctx}/task/auditedList">已审核任务</a></li>
			...
			<tbody>
				...
				<c:forEach items="${pagination.recordList}" var="task" varStatus="i">
					<tr>
						...
						<td>
							<c:choose>
								<c:when test="${task.status eq 12 or task.status eq 14}">
									<c:choose>
										<c:when test="${task.suggestion !=null and task.suggestion != ''}">
											<div class="moreInfo red J_hook_more clearfix">
												...
												<p class="none">${task.suggestion}</p>
											</div> 	
										</c:when>
										<c:otherwise>
											<div class="red">
						...
						<td class="edit">
							<ul class="f_ul">
								...
								<li></li>
								<c:choose>
									<c:when test="${task.type == 2}">
										<a href="javascript:detail('${ctx}/plan/planDetailStat/planReportView?planId=${task.planId}')">报告</a>
									</c:when>
									<c:otherwise>
										<a href="javascript:detail('${ctx}/taskDetailStat/overview?taskId=${task.taskId}')">报告</a>
									</c:otherwise>
								</c:choose>
