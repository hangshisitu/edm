		...
		search:function(options){
			...
			return this.each(function(){
				...
				$this.bind('click',function(){
					...
					//有节点处理，不过这里影响性能
					if(p.sortFn){p.sortFn.call($boxId);};
	...
	function initInput(ele,valueEle,json,subArr){
			...
			if(typeof json.length =='number'){
				for(i=0;i<json.length;i++){
					if(json[i].id==id || jQuery.inArray(json[i].id.toString(),id.split(','))!=-1){
						...
						if(typeof subArr == 'object'){
							if(json[i].info){
								var newarr=new Array(json[i].id,json[i].title,json[i].info);
							}else if(json[i].type){
								var newarr=new Array(json[i].id,json[i].title,json[i].type);
							}else{
				...
				for(key in json){
					var obj=json[key];
					for(i=0;i<obj.length;i++){
						if(obj[i].id==id || jQuery.inArray(obj[i].id.toString(),id.split(','))!=-1){
							textArr.push(obj[i].title);
							...
							if(typeof subArr == 'object'){
								if(obj[i].info){
									var newarr=new Array(obj[i].id,obj[i].title,obj[i].info);
								}else if(obj[i].type){
									var newarr=new Array(obj[i].id,obj[i].title,obj[i].type);
								}else{
									var newarr=new Array(obj[i].id,obj[i].title);
								}
								subArr.push(newarr);
							}	
						}
