...
public class Validates {
	...
	public static boolean clean(Object target) {
		if (isTarget(target)) {
			if (!Jsoup.isValid((String) target, Whitelist.none())) {
				return false;
			}
		}
		return true;
	...
	private static boolean number(Object target, Object min, Object max) {
		if (isTarget(target)) {
			...
			if (NumberUtils.isNumber(str) && !StringUtils.contains(str, ".")) {
				int len = String.valueOf(max).length();
				if (str.length() < len) {
					return true;
				}
				if (str.length() == len && str.compareTo(String.valueOf((StringUtils.startsWith(str, "-") ? min : max))) <= 0) {
					return true;
				}
			}
	...
	private static boolean isTarget(Object target) {
