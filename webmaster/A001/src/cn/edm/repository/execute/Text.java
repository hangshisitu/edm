...
public class Text {
	...
	public int mailPos(PathMap path, String fileId, MultipartFile upload, String charset, Integer[] propIds, Map<Integer, Prop> propMap, int mailPos) {
		...
		try {
			...
			writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(Webs.root() + filePath + "/map.txt"), "UTF-8"));
	...
	public void execute(String userId, MultipartFile upload, String charset, Integer[] propIds, Map<Integer, Prop> propMap, int mailPos, 
		...
		try {
			...
			Pos pos = new Pos();
			recipientDao.start(recipientDbs, recipientColls, userId, maxRecipientDbId, recipientDbIds, maxColls, pos);
			...
			while ((line = reader.readLine()) != null) {
				...
				if (recipientList.size() == batch) {
					recipientDao.save(recipientDbs, recipientColls, maxColls, maxRecipients, userId, recipientList, recipientDbIds, pos);
			...
			if (!Asserts.empty(copyMap)) {
                for (Integer[] arr : copyMap.values()) {
                    copyRecipientList.add(new Copy(userId, arr[0], arr[1], Copy.MODIFY));
                }
            }
