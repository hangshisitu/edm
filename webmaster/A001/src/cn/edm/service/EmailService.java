...
//		recipientService.save(recipient.getUserId(), maxRecipientDbId, recipientDbIds);
		...
		// CopyTrigger触发
        /*dao.save("CopyTag.save", new Copy(tag.getUserId(), tag.getDbId(), tag.getTagId(), Copy.MODIFY));*/
...
//        }
	...
	public Recipient get(Tag tag, String email, boolean parameter) {
		...
		Recipient recipient = tagDao.get(tag, email);
		if(recipient == null)
...
//			
//			RichTextString = tagDao.get(tag, email);
//
////			if (parameter) {
////				List<Recipient> recipientDbIds = recipientService.find(tag.getUserId());
////				String parameters = recipientDao.getParameters(tag.getUserId(), email, recipientDbIds);
...
//		}
	...
	public Page<Recipient> search(Page<Recipient> page, MapBean mb,
			Map<Integer, String> mapper, Integer dbId, String userId,
		...
		DBCollection coll = tagDao.getColl(dbId, userId, tagId);
